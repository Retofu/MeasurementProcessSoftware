...
Адрес каждого регистра обозначается шестнадцатеричным числом. Каждый регистр в ARM имеет длину 32 бита. Регистр имеет поля - наборы ячеек, отвечающих за реализацию функций. У регистра, полей и значений полей есть режимы доступа.
...
Доступ к регистру специального назначения может быть выполнен через операцию обращения к регистру:
[source, cpp, linenums]
----
GPIOC::ODR::ODR8::High::Set();
----
